          ...
          <div className="mt-32 overflow-hidden">
            ...
            >
              <h3 className="text-3xl font-bold text-gray-900 relative z-10 inline-block">
                Key Applications
                <div className="absolute -bottom-2 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-green-500 to-transparent opacity-50" />
              </h3>
            </motion.div>
              ...
              <div className="flex overflow-hidden">
                ...
                >
                  {[...data.applications, ...data.applications, ...data.applications].map((app, index) => (
                    <div
                      key={index}
                      className="group relative h-40 w-64 flex-shrink-0 flex flex-col justify-center items-center p-6 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-[0_20px_40px_-15px_rgba(16,185,129,0.2)] hover:border-green-100 transition-all duration-300"
                    >
                       <div className="absolute inset-0 bg-gradient-to-br from-green-50/0 to-green-50/0 group-hover:from-green-50/50 group-hover:to-emerald-50/30 rounded-2xl transition-all duration-500" />
                      
                      {/* Decorative Icon */}
                      <div className="mb-4 p-3 bg-green-50 rounded-full group-hover:bg-green-100 group-hover:scale-110 transition-all duration-300">
                        <Sparkles className="w-5 h-5 text-green-600/70 group-hover:text-green-600 transition-colors" />
                      </div>

                      <span className="text-lg font-bold text-gray-700 text-center group-hover:text-green-800 transition-colors relative z-10">
                        {app}
                      </span>
                    </div>
                  ))}
